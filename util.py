...
class Util:
    ...
    def show_this_weeks_games(games: list[dict], user_forecast_values: bool = False, condensed_version:bool = False, skip_print:bool = False) -> str:
        ...
        if len(upcoming_games) > 0:
            ...
            if len(matchups) > 0:
                result_str += f'Forecasting for games in week {int(THIS_NFL_WEEK) - 35} of the {CURRENT_SEASON_YEAR} NFL season...\n'
                result_str += f'Matchup\t\tDate\t\tHome\tElo Prediction\t{"Your" if user_forecast_values else "FiveThirtyEight"} Prediction\tAway\tElo Prediction\t{"Your" if user_forecast_values else "FiveThirtyEight"} Prediction\n'
